...
-- All functions use security definer + is_teacher_or_admin() check.
...
-- ── 2. Update user (role, display_name, class_id) ──
...
begin
  ...
  if p_display_name is not null then
    update public.app_users set display_name = p_display_name, updated_at = now()
    where id = p_user_id;
  end if;
  ...
  if p_class_id is not null then
    update public.app_users set class_id = p_class_id, updated_at = now()
    where id = p_user_id;

    -- Sync class_name for backward compat
    update public.app_users u
    set class_name = (select name from public.classes where id = p_class_id)
...
begin
  ...
  -- Delete Supabase auth identity and user
...
-- ── 4. Reset user password to pasraman-{username} ──
...
begin
  ...
  -- Update app_users password_hash
  update public.app_users
  set password_hash = crypt(v_new_password, gen_salt('bf')), updated_at = now()
  where id = p_user_id;
  ...
  return jsonb_build_object(
    'ok', true,
    'message', format('Password %s direset menjadi pasraman-%s', v_username, v_username),
