import { Chapter, StudentInfo, StudentProgressMap, TaskInboxItem } from './types';
...
// --- Student Progress ---
export const STUDENT_PROGRESS: StudentProgressMap = {
  'bab-1': {
    preTestCompleted: true,
    preTestScore: 66,
    materialCompleted: true,
    completedTaskIds: ['t1-1'],
    postTestMandatoryCompleted: true,
    postTestMandatoryScore: 80,
    postTestOptionalSubmitted: true,
    postTestOptionalLink: 'https://youtube.com/watch?v=example1',
    postTestOptionalScore: 85,
  },
  'bab-2': {
    preTestCompleted: true,
    preTestScore: 100,
    materialCompleted: true,
    completedTaskIds: [],
    postTestMandatoryCompleted: false,
    postTestMandatoryScore: null,
    postTestOptionalSubmitted: false,
    postTestOptionalLink: null,
    postTestOptionalScore: null,
  },
  'bab-3': {
    preTestCompleted: false, // no pre-test, so false
    preTestScore: null,
...
// --- Task Inbox (computed from chapters + progress) ---
export function getTaskInbox(): TaskInboxItem[] {
  const items: TaskInboxItem[] = [];
  for (const chapter of CHAPTERS) {
    const progress = STUDENT_PROGRESS[chapter.id];
    ...
    // Post-test optional submissions
    if (chapter.postTestOptional && progress.postTestOptionalSubmitted) {
      items.push({
        chapterId: chapter.id,
        chapterTitle: chapter.title,
        task: {
        ...
        status: progress.postTestOptionalScore !== null ? 'graded' : 'submitted',
        score: progress.postTestOptionalScore,
      });
    }
  }
  return items;
